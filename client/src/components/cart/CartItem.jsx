...
export default function CartItem({ item }) {
  ...
  // Cart items are stored as flat product objects with `id` and `qty`
  // Supabase images are in `product_images`, legacy in `images`
  ...
  return (
    <div className="flex gap-4 p-3 md:p-4 bg-white rounded-xl shadow-sm border border-gray-100 mb-3 items-center">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-sm font-bold text-primary-600">{formatPrice(item.price)}</div>
      </div>

      <div className="flex flex-col items-end gap-2">
        <div className="text-sm font-bold text-gray-800">{formatPrice(item.price * qty)}</div>
